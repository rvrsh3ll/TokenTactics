...
function Invoke-DeployCaptureServer {
    ...
    .EXAMPLE
        Invoke-DeployCaptureServer -ResourceGroup Myresourcegroup -location eastus -vmName codecapture -vmPublidDNSName mycodecaptureserver -pubKey ./mykey.pub
    #>
    ...
    [Parameter(Mandatory=$False, Default="codecapture")]
    [String]$vmName,
...
Start-Sleep -Seconds 5
Write-Output "az vm run-command invoke -g $ResourceGroup -n $vmName --command-id RunShellScript --scripts 'sudo -i -u git clone https://github.com/rvrsh3ll/TokenTactics.git /home/azureuser/TokenTactics'"
